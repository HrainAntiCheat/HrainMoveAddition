...
public class Fly extends MovementCheck {
    ...
    //TODO perhaps have a ground checking mode? clientside vs serverside? doing it server side can allow roundSpoof to not cancel, yet Gravity will still catch groundspoof-based flys
    ...
    public Fly() {
        super("fly", true, 0, 10, 0.995, 5000, "&7%player% 没能绕过 fly, VL: %vl%", null);
    ...
    private boolean isOnBoat(Player p, Location loc) {
        Set<Entity> trackedEntities = HrainAC.getLagCompensator().getPositionTrackedEntities();
        int ping = ServerUtils.getPing(p);
        for(Entity entity : trackedEntities) {
            if (entity instanceof Boat) {
                AABB boatBB = WrappedEntity.getWrappedEntity(entity).getCollisionBox(HrainAC.getLagCompensator().getHistoryLocation(ping, entity).toVector());
                AABB feet = new AABB(
    ...
    private double computeVerticalWaterFlowForce(double y, HrainACPlayer pp) {
