...
package me.nuymakstone.hrainac.check.tick;
 ...
 */
public class Timer extends MovementCheck implements Listener {
    ...
    public Timer() {
        super("Timer", true, 10, 50, 0.995, 10000, "%player% failed timer. VL: %vl%, ping: %ping%, TPS: %tps%", null);
    ...
    protected void check(MoveEvent event) {
        ...
        if (pp.getCurrentTick() - lastBigTeleportTime.getOrDefault(p.getUniqueId(), 0L) < WARM_UP) {
            if(DEBUG)
                p.sendMessage(ChatColor.GRAY + "Timer check warming up. Please wait a moment...");
    ...
    public void onWorldChange(PlayerChangedWorldEvent e) {
        lastBigTeleportTime.put(e.getPlayer().getUniqueId(), HrainAC.getHrainACPlayer(e.getPlayer()).getCurrentTick());
    }
