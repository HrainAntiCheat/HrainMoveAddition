...
public class EntityInteractDirection extends EntityInteractionCheck {
    ...
    public EntityInteractDirection() {
        super("entityinteractdirection", true, 0, 10, 0.9, 5000, "%player% 没能绕过 entity interact direction, VL: %vl%", null);
    ...
    protected void check(InteractEntityEvent e) {
        ...
        Vector pos;
        if(pp.isInVehicle()) {
            pos = HrainAC.getLagCompensator().getHistoryLocation(ping, p).toVector();
        ...
        Location victimLoc;
        if(LAG_COMPENSATION)
            victimLoc = HrainAC.getLagCompensator().getHistoryLocation(ping, victimEntity);
        else
