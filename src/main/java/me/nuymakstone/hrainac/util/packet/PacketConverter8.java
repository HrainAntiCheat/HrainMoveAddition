...
package me.nuymakstone.hrainac.util.packet;
...
import me.nuymakstone.hrainac.wrap.packet.WrappedPacket8;
...
public final class PacketConverter8 {
    ...
    public static Event packetInboundToEvent(Object packet, Player p, HrainACPlayer pp) {
    ...
    private static HrainACAsyncPlayerTeleportEvent packetToPlayerTeleportEvent(PacketPlayOutPosition packet, Player p) {
        ...
        try {
            ...
            return new HrainACAsyncPlayerTeleportEvent(p, x, y, z, yaw, pitch);
    ...
    private static HrainACAsyncPlayerAbilitiesEvent packetToPlayerAbilitiesOutEvent(PacketPlayOutAbilities packet, Player p) {
        ...
        try {
            ...
            return new HrainACAsyncPlayerAbilitiesEvent(p, invulnerable, allowedToFly, flying, creativeMode, flySpeed, walkSpeed);
        }
        catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static HrainACAsyncPlayerMetadataEvent packetToPlayerMetadataEvent(PacketPlayOutEntityMetadata packet, Player p) {
        PacketDataSerializer serializer = new PacketDataSerializer(Unpooled.buffer(0));
        try {
            packet.b(serializer);
            int id = serializer.e();
            ...
            return new HrainACAsyncPlayerMetadataEvent(p, wrappedMetaData);
        }
        catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static HrainACAsyncPlayerVelocityChangeEvent packetToVelocityEvent(Packet packet, Player p) {
        if(packet instanceof PacketPlayOutExplosion) {
            ...
            if(velocity.lengthSquared() == 0)
                return null;
            return new HrainACAsyncPlayerVelocityChangeEvent(velocity, p, true);
        }
            ...
            return new HrainACAsyncPlayerVelocityChangeEvent(velocity, p, false);
        }
        return null;
    }

    private static Event packetToPosEvent(PacketPlayInFlying packet, Player p, HrainACPlayer pp) {
        //default position
    ...
    private static InteractEntityEvent packetToInterEvent(PacketPlayInUseEntity packet, Player p, HrainACPlayer pp) {
    ...
    private static Event packetToDigEvent(PacketPlayInBlockDig packet, Player p, HrainACPlayer pp) {
    ...
    private static CustomPayLoadEvent packetToPayloadEvent(PacketPlayInCustomPayload packet, Player p, HrainACPlayer pp) {
        return null;
    }

    private static AbilitiesEvent packetToAbilitiesEvent(PacketPlayInAbilities packet, Player p, HrainACPlayer pp) {
    ...
    private static Event packetToUseEvent(PacketPlayInBlockPlace packet, Player p, HrainACPlayer pp) {
    ...
    private static ArmSwingEvent packetToArmSwingEvent(PacketPlayInArmAnimation packet, Player p, HrainACPlayer pp) {
        return new ArmSwingEvent(p, pp, 0, new WrappedPacket8(packet, WrappedPacket.PacketType.ARM_ANIMATION));
    }

    private static ItemSwitchEvent packetToItemSwitchEvent(PacketPlayInHeldItemSlot packet, Player p, HrainACPlayer pp) {
        return new ItemSwitchEvent(p, pp, packet.a(), new WrappedPacket8(packet, WrappedPacket.PacketType.HELD_ITEM_SLOT));
    }

    private static PlayerActionEvent packetToPlayerActionEvent(PacketPlayInEntityAction packet, Player p, HrainACPlayer pp) {
    ...
    private static ClickInventoryEvent packetToWindowClickEvent(PacketPlayInWindowClick packet, Player p, HrainACPlayer pp) {
    ...
    private static OpenPlayerInventoryEvent packetToOpenWindowEvent(PacketPlayInClientCommand packet, Player p, HrainACPlayer pp) {
    ...
    private static CloseInventoryEvent packetToCloseWindowEvent(PacketPlayInCloseWindow packet, Player p, HrainACPlayer pp) {
