...
public class AdjacentBlocks {
     ...
     * since that flag can be spoofed by the client. HrainMoveAddition's definition of being on
    ...
    public static boolean onGroundReally(Location loc, double yVelocity, boolean ignoreInGround, double feetDepth, HrainACPlayer pp) {
        ...
        for (Block block : blocks) {
            ...
            if (block.isLiquid() || (!bNMS.isSolid() && HrainMoveAddition.getServerVersion() == 8))
                continue;
            if (bNMS.isColliding(underFeet)) {
                ...
                if (ignoreInGround) {
                    ...
                    for (Block block1 : AdjacentBlocks.getBlocksInLocation(loc)) {
                        ...
                        if (block1.isLiquid() || (!bNMS1.isSolid() && HrainMoveAddition.getServerVersion() == 8) ||
    ...
    public static boolean blockNearbyIsSolid(Location loc, boolean HrainACDefinition) {
        Location check = loc.clone();
        Set<Block> sample = new HashSet<>();
        ...
        for (Block b : sample) {
            if (b == null)
                continue;
            if (HrainACDefinition) {
                if (WrappedBlock.getWrappedBlock(b, HrainMoveAddition.getServerVersion()).isSolid())
                    return true;
