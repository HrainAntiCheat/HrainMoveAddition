    ...
    public boolean process(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        for (Check check : HrainAC.getCheckManager().getChecks()) {
            if (check.getName().equalsIgnoreCase(args[2])) {
                sender.sendMessage(ChatColor.AQUA + target.getName() + "的VL " + check.getName() + ": " + HrainAC.getHrainACPlayer(target).getVL(check));
                return true;
            }
        }
        sender.sendMessage(ChatColor.RED + "未知检查 \"" + args[2] + "\"");
        return true;
    }
}
