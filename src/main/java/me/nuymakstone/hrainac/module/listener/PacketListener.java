...
package me.nuymakstone.hrainac.module.listener;

import me.nuymakstone.hrainac.HrainMoveAddition;
import me.nuymakstone.hrainac.module.PacketHandler;
...
public abstract class PacketListener {
    ...
    private Thread HrainACAsyncCheckThread;
    ...
    public void disable() {
        running = false;
        if(async) {
            synchronized (HrainACAsyncCheckThread) {
                HrainACAsyncCheckThread.notify();
            }
        }
        removeAll();
    }
    ...
    private void prepareAsync() {
        ...
        HrainACAsyncCheckThread = new Thread(() -> {
            while(running) {
        ...
        HrainACAsyncCheckThread.setName("HrainMoveAddition Async Check Thread");
    }

    private void printPacketErrorInformation(Object packet, Player p) {
        String packetName = packet.getClass().getSimpleName();
        String pName = p == null ? "(NULL)" : "\"" + p.getName() + "\"";
        System.err.println("HrainMoveAddition (version " + HrainMoveAddition.BUILD_NAME + ") has encountered an error while processing " + packetName + " for player " + pName);
