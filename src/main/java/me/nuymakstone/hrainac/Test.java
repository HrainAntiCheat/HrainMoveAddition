...
public class Test {
    ...
    public Test(HrainMoveAddition HrainAC) {
    ...
    public void start(HrainMoveAddition HrainAC) {
        ...
        HrainAC.getPacketHandler().addPacketAdapterOutbound(adapter);
    }
        ...
        PacketAdapter adapter = new PacketAdapter() {
            ...
            public void run(Object packet, Player player) {
                if(packet instanceof PacketPlayOutExplosion) {
                    ...
                    Bukkit.getScheduler().runTask(HrainAC, new Runnable() {
                        @Override
                        public void run() {
                            Bukkit.getServer().getPluginManager().callEvent(new HrainACAsyncPlayerVelocityChangeEvent(velocity, player, true));
                        }
                    });
                }
                if(packet instanceof PacketPlayOutEntityVelocity) {
                    ...
                    Bukkit.getScheduler().runTask(HrainAC, new Runnable() {
                        @Override
                        public void run() {
                            Bukkit.getServer().getPluginManager().callEvent(new HrainACAsyncPlayerVelocityChangeEvent(velocity, player, false));
                        }
                    });
                }
            }
