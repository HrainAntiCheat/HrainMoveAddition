...
public class MoveEvent extends Event {
    ...
    //Also, movement checks utilizing hit slowdown must use this field and not the one in HrainACPlayer
    //since HrainACPlayer's will always be false because HrainACPlayer will tick on MoveEvent#preProcess()
    ...
    public MoveEvent(Player p, Location update, boolean onGround, HrainACPlayer pp, WrappedPacket packet, boolean updatePos, boolean updateRot) {
        super(p, pp, packet);
        toLocation = update;
    ...
    public boolean preProcess() {
        ...
        Vector lastPos = pp.getPosition(); //set this now, because HrainACPlayer#getPosition() will change after teleport
    ...
    private boolean testPistonPush(Vector from, Vector to, HrainACPlayer pp) {
        ...
        if(x || y || z) {
            return HrainAC.getLagCompensator().testNearPiston(to, pp.getWorld(), ServerUtils.getPing(pp.getPlayer()));
        }
        return false;
    }
    ...
    private float computeFriction() {
        ...
        if(pp.isInWater() && !flying) {
            ...
            if(HrainAC.getServerVersion() > 7 && boots != null) {
