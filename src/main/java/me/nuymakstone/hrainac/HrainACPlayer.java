...
import me.nuymakstone.hrainac.wrap.entity.human.WrappedEntityHuman;
...
public class HrainACPlayer {
    ...
    HrainACPlayer(Player p, HrainMoveAddition HrainAC) {
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask(HrainAC, new Runnable() {
            @Override
            public void run() {
    ...
    //Returns the current predicted position of the HrainACPlayer, assuming no user input.
    ...
    private void predictNextPosition() {
        ...
        //a.highlight(HrainMoveAddition, getWorld(), 0.299);
        ...
        if(newTouchedMats.contains(Material.WEB)) {
            ...
            if(HrainAC.getServerVersion() == 8 && !isSneaking() && b != null && b.getType() == Material.SLIME_BLOCK && move.getY() < 0) {
    ...
    public void kickPlayer(String reason) {
        online = false;
        Bukkit.getScheduler().scheduleSyncDelayedTask(HrainAC, () -> p.kickPlayer(reason), 0L);
    }

    //safely teleport player
    public void teleport(Location location, PlayerTeleportEvent.TeleportCause teleportCause) {
        Bukkit.getScheduler().scheduleSyncDelayedTask(HrainAC, () -> p.teleport(location, teleportCause), 0L);
    }

    //safely force player to release item (bow, consumable, or sword blocking)
    public void releaseItem() {
        Bukkit.getScheduler().scheduleSyncDelayedTask(HrainAC, () -> ((WrappedEntityHuman)WrappedEntity.getWrappedEntity(p)).releaseItem());
        sendSimulatedAction(() -> {
            setBlocking(false);
            setPullingBow(false);
